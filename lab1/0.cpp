...
class Dice {
 public:
  ...
  unsigned roll() { return dstr(reng); }
...
class ThreeDicePool {
 public:
  ...
  unsigned roll() { return dstr_1(reng_1) + dstr_2(reng_2) + dstr_3(reng_3); }
...
double expected_value(Dice &d, unsigned number_of_rolls = 1) {
  auto accum = 0llu;
  for (unsigned cnt = 0; cnt != number_of_rolls; ++cnt) accum += d.roll();
  return static_cast<double>(accum) / static_cast<double>(number_of_rolls);
}

double expected_value(ThreeDicePool &tdp, unsigned number_of_rolls = 1) {
  auto accum = 0llu;
  for (unsigned cnt = 0; cnt != number_of_rolls; ++cnt) accum += tdp.roll();
  return static_cast<double>(accum) / static_cast<double>(number_of_rolls);
}
